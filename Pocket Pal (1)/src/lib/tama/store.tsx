...
type Action =
  ...
  | { type: "_applyReply"; result: InferenceResult; isFreeText: boolean }
  | {
  ...
  switch (action.type) {
    ...
    case "_applyReply": {
      ...
      return {
        ...
        memory: nextMemory,
        pendingInferences: result.proposedInferences,
        // quick-answer path opens say-more; free-text path closes it
    ...
    case "clearCrisis":
      return { ...state, crisis: { active: false, triggeredAt: null }, crisisDemoPreview: false };
    ...
    case "sendPetSignal": {
      const sig = petSignalService.buildSignal("quiet_spark", state.petName, state.userName);
        ...
        conversation: [
          {
            id: "m0",
            from: "pocket",
            text: `hi ${cleanUser.toLowerCase()}. i'm ${cleanPet.toLowerCase()}. small check-in?`,
            at: new Date().toISOString(),
          },
        ],
      };
...
export function TamaProvider({ children }: { children: ReactNode }) {
  ...
  const wrappedDispatch = useCallback<React.Dispatch<Action>>((action) => {
    if (action.type === "quickAnswer") {
      ...
      // If the sync crisis check just flipped crisis on, stop.
      ...
      ).then((result) => dispatch({ type: "_applyReply", result, isFreeText: false }));
      return;
    }
    if (action.type === "sayMore") {
      ...
      void getPocketReply(
        {
          quick: snap.lastQuick?.key ?? null,
          quickLabel: snap.lastQuick?.label ?? null,
          freeText: text,
        },
        {
          wellbeing: snap.wellbeing,
      ...
      ).then((result) => dispatch({ type: "_applyReply", result, isFreeText: true }));
      return;
    }
    dispatch(action);
  }, []);
